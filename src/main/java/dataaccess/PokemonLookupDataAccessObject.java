...
public class PokemonLookupDataAccessObject implements PokemonLookupDataAccessInterface {
    ...
    public Pokemon getPokemon(String name) throws IOException {
        ...
        final ArrayList<String> hyphens = new ArrayList<>(Arrays.asList(
                "ho-oh", "jangmo-o", "kommo-o", "hakamo-o", "porygon-z",
                "ting-lu", "chi-yu", "wo-chien", "chien-pao"
        ));
        final String api1 = "https://pokeapi.co/api/v2/pokemon/";
        final String api2 = "https://pokeapi.co/api/v2/pokemon-species/";
        OkHttpClient client;
        Request request1;
        Request request2;
        ...
        if (name.split("-").length == 2 && hyphens.contains(name)) {
            ...
            final int index = name.indexOf(dash);
            if (index != -1) {
        ...
        try (Response response = client.newCall(request1).execute()) {
            ...
            try (Response response2 = client.newCall(request2).execute()) {
                ...
                final String responseBody2 = response2.body().string();
                final JSONObject json2 = new JSONObject(responseBody2);
